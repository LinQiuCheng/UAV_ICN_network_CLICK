...
#include"ForwardTableV2.hh"
#include"struct.hh"
#include <click/args.hh>
#include <click/error.hh>
#include <click/etheraddress.hh>
#define EXPIRE_TIME 500
#define MAX_ROUTE_COUNT 5
#define MAX_HOP 6

ForwardTableV2::ForwardTableV2(){
    expiretime=EXPIRE_TIME;
    memset(broadcast_addr,0xff,6);
	printf("init ForwardTableV2 suc\n\n");
...
int
ForwardTableV2::configure(Vector<String> &conf, ErrorHandler *errh)
{
...
void
ForwardTableV2::push(int port, Packet *packet)
{
	...
	if(port == 0){ //输入口 0 进入 该由本节点产生数据的GET包
        ...
        if(forwardtablev2.count(node)==0){  //没有该节点的路由
            
            forwardtablev2_item insert;
            insert.create_time=GetTickCount();
            insert.metric=MAX_HOP - info.ttl;
            insert.route_count=0;
            memcpy(insert.next_hop,ether_hd.ether_shost,6);
            forwardtablev2.insert(std::pair< uint16_t, forwardtablev2_item>(node,insert));
        }
        else{
            
            std::pair<multimap_iterator,multimap_iterator> it =
                forwardtablev2.equal_range(node);
        ...
        if(forwardtablev2.count(key)==0){ //没有到数据生产者的相关路由
            ...
            //printf("found route\n");
            std::pair<multimap_iterator,multimap_iterator> it =
                forwardtablev2.equal_range(key);
            /*
            multimap_iterator k;
            ...
            */
            if((it.first->second.create_time + EXPIRE_TIME < GetTickCount())||it.first->second.route_count>MAX_ROUTE_COUNT){//过期了 
                forwardtablev2.erase(it.first); //删除该项
                memcpy(ether_hd.ether_dhost,broadcast_addr,6); //给予目的地址为广播地址
                memcpy((unsigned char *)packet->data(),&ether_hd,14);
                printf("route over time or route count max\n");
                output(1).push(packet);
            }
            else
            {
                it.first->second.route_count++;
        ...
        if(node != nid){ //不是本节点自身发出的get
            forwardtablev2_item insert;
            if(forwardtablev2.count(node)==0){ //没有到该节点的路由，可以由get学习得到
                insert.create_time=GetTickCount();
                insert.metric=MAX_HOP - info.ttl;
                insert.route_count=0;
		...
		if(node!=key) //缓存节点发来的数据
        {
            ...
            if(forwardtablev2.count(node)==0){
                forwardtablev2_item insert;
                insert.create_time=GetTickCount();
                insert.metric=MAX_HOP - info.ttl;
                insert.route_count=0;
                ...
                if(it.first->second.metric>=(MAX_HOP-info.ttl)){ //如果路径更好，替换  路径一样，更新路由表项生存时间
                    it.first->second.create_time=GetTickCount();
                    it.first->second.metric=MAX_HOP - info.ttl;
                    if(it.first->second.route_count>0){
                        it.first->second.route_count--;
                    }
                    if(strncmp((char *)it.first->second.next_hop,(char *)record.ether_shost,6)!=0) //下一跳有变化
                    {    
...
unsigned long ForwardTableV2::GetTickCount(){
    struct timespec ts;
...
CLICK_ENDDECLS
EXPORT_ELEMENT(ForwardTableV2)
ELEMENT_MT_SAFE(ForwardTableV2)
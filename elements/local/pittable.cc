...
int
PitTable::configure(Vector<String> &conf, ErrorHandler *errh)
{
...
PitTable::push(int port, Packet *packet)
{
	...
	if(port==0){      //输入口 0 进入 GET包
        ...
        if(((info.sid&0xffff0000)>>16)==nid) { //获取本节点数据的GET
            if(pitTable.count(info.sid)==0) // 没有找到相同sid的表项
            {
                ValueNode add;
                ...
                pitTable.insert(std::pair< uint32_t, ValueNode>(info.sid,add));
                ...
                std::pair<multimap_iterator,multimap_iterator> it = 
                            pitTable.equal_range(info.sid);   //使用迭代器找出所有结果
                multimap_iterator k;
        ...
        if(pitTable.count(data.sid)==0){ //没找到该项 ，将该包丢弃 
            printf("not find item discard\n"); 
            output(3).push(packet);
        }
        else{
            std::pair<multimap_iterator,multimap_iterator> it = 
                           pitTable.equal_range(data.sid);   //使用迭代器找出所有结果
            //int flag1=0,flag2=0; 
            if(data.split==0&&data.offset==0){ //无分片的数据包
                printf("Not fragment\n");
                ...
                while(it.first!=it.second){
                    ...
                    packet1->push(14+sizeof(data)+data.length);
                    memcpy((unsigned char*)packet1->data(),(unsigned char*)packet->data(),+14+sizeof(data)+data.length);
                    if(it.first->second.offset==0&&it.first->second.split==0){
                        if(strncmp((char*)it.first->second.ether_shost,(char *)self_mac.ether_shost,6)==0){   // 节点本身请求的data
                            ...
                            output(1).push(packet1);
                        }
                        it.first=pitTable.erase(it.first);
                        continue;
                    }
                    it.first++;
                }
                ...
                while(it.first!=it.second){
                    if(it.first->second.offset==data.offset&&it.first->second.split==data.split){//请求单片的表项
                        String _data="";
                        Packet * packet1 = Packet::make(Packet::default_headroom, 
                        _data.data(), _data.length(), 0);     
                        packet1->push(14+sizeof(data)+data.length);
        ...
        std::pair<multimap_iterator,multimap_iterator> it = 
                           pitTable.equal_range(SID);   //使用迭代器找出所有结果
        while(it.first!=it.second){
            if(it.first->second.offset==0&&it.first->second.split==0){
                it.first=pitTable.erase(it.first);
                continue;
